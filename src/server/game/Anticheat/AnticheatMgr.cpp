...
void AnticheatMgr::TeleportHackDetection(Player* player, MovementInfo movementInfo)
{
    ...
    {
        TC_LOG_DEBUG("entities.player.character", "AnticheatMgr:: Teleport - Hack detected player GUID (low) %u", player->GetGUIDLow());
...
void AnticheatMgr::StartHackDetection(Player* player, MovementInfo movementInfo, uint32 opcode)
{
	...
	if (player->GetSession()->GetSecurity() >= AccountTypes(sWorld->getIntConfig(CONFIG_MOVEMENT_CHECKS_ACCESSLEVEL)))
		return;
	...
	{
		player->wasInFlight = true;
		m_Players[key].SetLastMovementInfo(movementInfo);
		m_Players[key].SetLastOpcode(opcode);
		return;
	}

	// if player was in flight, transport or vehicle ignore the anticheat untill he lands
	if (player->wasInFlight)
	{
		player->wasInFlight = false;
		return;
	}
...
void AnticheatMgr::BuildReport(Player* player, uint8 reportType)
{
	...
	if (m_Players[key].GetTotalReports() >= sWorld->getIntConfig(CONFIG_ANTICHEAT_REPORTS_AMOUNT) || !MustCheckTempReports(reportType))
     {
         uint8 action = sWorld->getIntConfig(CONFIG_ANTICHEAT_ACTION);
         if (action <= 2)
         {
             switch (action)
             {
                 case NONE:
                     break;
                 case KICK:
                     player->GetSession()->KickPlayer("PASSIVEANTICHEAT: Player Kicked for exceeding maximum reports (Hacking)");
                     TC_LOG_WARN("anticheat", "PASSIVEANTICHEAT: [Player: %s (Guid: %u, Account: %u)] Kicked for exceeding maximum reports", player->GetName().c_str(), player->GetGUIDLow(), player->GetSession()->GetAccountId());
                 ...
                 case BAN:
                     ...
                     duration << sWorld->getIntConfig(CONFIG_ANTICHEAT_BAN_DURATION) << "s";
                     std::string accountName;
                     AccountMgr::GetName(player->GetSession()->GetAccountId(), accountName);
                     std::stringstream banReason;
                     banReason << "Passive Anticheat Violation";
                     sWorld->BanAccount(BAN_ACCOUNT, accountName, duration.str(), banReason.str(),"Server");
                     TC_LOG_WARN("anticheat", "PASSIVEANTICHEAT: [Player: %s (Guid: %u, Account: %u)] Banned for exceeding maximum reports", player->GetName().c_str(), player->GetGUIDLow(), player->GetSession()->GetAccountId());
 
                     if (sWorld->getBoolConfig(CONFIG_ANTICHEAT_ACTION_ANNOUNCE))
                         sWorld->SendWorldText(LANG_ANTICHEAT_BANNED_PLAYER, player->GetName().c_str());
                     break;
             }
         }
         else
             TC_LOG_ERROR("misc", "Invalid action specified for Anticheat.Action in the config");
     }

	if (m_Players[key].GetTotalReports() > sWorld->getIntConfig(CONFIG_ANTICHEAT_REPORTS_INGAME_NOTIFICATION) || !MustCheckTempReports(reportType))
	{
