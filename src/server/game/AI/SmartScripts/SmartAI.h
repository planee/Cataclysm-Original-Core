...
class SmartAI : public CreatureAI
{
    public:
        ...
        void StartPath(bool run = false, uint32 path = 0, bool repeat = false, Unit* invoker = NULL, bool isCyclic = false, bool isCatmullRom = false, uint32 p_MovementType = POINT_MOTION_TYPE);
        void StartCustomPath(uint32 pathId, bool run, bool repeat, bool isCyclic, bool isCatmullRom, CustomWaypointPath* path);
        ...
        void OnAuraApplied(uint32 p_SpellID);

        /// OnAuraApplied - When Unit gets aura removed
        /// @p_SpellID - SpellID of Aura
        void OnAuraRemoved(uint32 p_SpellID);

        /// Called when this unit leaves a vehicle
        void OnLeaveVehicle(Unit* p_Unit);

        /// Called when this unit requests to exit a vehicle
        void OnVehicleExitRequested(Unit* p_Unit);

        /// Called when player tries to talk to creature
        bool IsIgnoringStopOnInteraction();
    ...
    private:
        ...
        bool m_IsIgnoringStopOnInteraction;
};
