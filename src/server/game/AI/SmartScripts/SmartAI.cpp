...
SmartAI::SmartAI(Creature* c) : CreatureAI(c)
{
	...
	isCatmullRomWP = false;
	isCyclicWP = false;
    m_MovementType = POINT_MOTION_TYPE;
    ...
    m_IsIgnoringStopOnInteraction = false;

    m_Script = new SmartScript(me);
}
...
void SmartAI::StartPath(bool run, uint32 path, bool repeat, Unit* /*invoker*/, bool isCyclic, bool isCatmullRom, uint32 p_MovementType)
{
	...
	isCatmullRomWP = isCatmullRom;
	isCyclicWP = isCyclic;
    m_MovementType = p_MovementType;
    SetRun(run);

    if (WayPoint* wp = GetNextWayPoint())
    {
        me->GetPosition(&mLastOOCPos);
        me->GetMotionMaster()->MovePoint(wp->id, wp->x, wp->y, wp->z, true, 0.0f, false, isCyclicWP, isCatmullRomWP);
        GetScript()->ProcessEventsFor(SMART_EVENT_WAYPOINT_START, NULL, wp->id, GetScript()->GetPathId());
    }
}

void SmartAI::StartCustomPath(uint32 pathId, bool run, bool repeat, bool isCyclic, bool isCatmullRom, CustomWaypointPath* path)
{
    SetRun(run);
    AddEscortState(SMART_ESCORT_ESCORTING);
    me->GetMotionMaster()->MoveCustomPath(pathId, repeat, isCyclic, isCatmullRom, path);
}
...
void SmartAI::ResumePath()
{
    SetRun(mRun);
    if (mLastWP)
        me->GetMotionMaster()->MovePoint(mLastWP->id, mLastWP->x, mLastWP->y, mLastWP->z, true, 0.0f, false, isCyclicWP, isCatmullRomWP);
}
...
void SmartAI::ReturnToLastOOCPos()
{
    ...
    if (me->GetMotionMaster()->GetMotionSlotType(MOTION_SLOT_ACTIVE) != CUSTOM_WAYPOINT_MOTION_TYPE)
    {
        SetRun(mRun);
...
{
    if (me->getVictim() && me->GetExactDist(me->getVictim()) < 30.0f && HasEscortState(SMART_ESCORT_ESCORTING))
        return;
    ...
    {
       // RemoveAuras(); // Is this necessary? @Tofigure
        if (!_EnterEvadeMode())
            return;
...
bool SmartAI::CanAIAttack(const Unit* /*who*/) const
{
    ...
    if (me->GetReactState() == REACT_PASSIVE || !IsAIControlled())
        return false;
...
void SmartAI::EnterCombat(Unit* enemy)
{
    ...
    /// I'm not so sure about this, but it might be needed
    GetScript()->mLastInvoker = enemy->GetGUID();
...
void SmartAI::AttackStart(Unit* who)
{
    ...
    if (who)
        if (who->GetGUID() == me->LastCharmerGUID)
            return;
...
void SmartAI::OnAuraApplied(uint32 p_SpellID)
{
    GetScript()->ProcessEventsFor(SMART_EVENT_AURA_APPLIED, nullptr, p_SpellID);
}

void SmartAI::OnAuraRemoved(uint32 p_SpellID)
{
    GetScript()->ProcessEventsFor(SMART_EVENT_AURA_REMOVED, nullptr, p_SpellID);
}

void SmartAI::OnLeaveVehicle(Unit* p_Unit)
{
    GetScript()->ProcessEventsFor(SMART_EVENT_AFTER_PASSENGER_REMOVED, p_Unit);
}

void SmartAI::OnVehicleExitRequested(Unit* p_Unit)
{
    GetScript()->ProcessEventsFor(SMART_EVENT_VEHICLE_EXIT_REQUSTED, p_Unit);
}

bool SmartAI::IsIgnoringStopOnInteraction()
{
    return m_IsIgnoringStopOnInteraction;
}
...
class SmartTrigger : public AreaTriggerScript
{
    public:
        ...
        bool OnTrigger(Player* player, AreaTriggerEntry const* trigger) override
        {
            ...
            SmartScript script(trigger, player);
