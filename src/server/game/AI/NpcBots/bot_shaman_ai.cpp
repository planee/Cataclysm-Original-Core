...
public:
    ...
    struct shaman_botAI : public bot_minion_ai
    {
            ...
            for (uint8 i = 0; i != MAX_TOTEMS; ++i)
            {
                if (_totems[i].first != 0)
                {
                        ...
                        me->GetDistance2d(_totems[i].second.pos.m_positionX, _totems[i].second.pos.m_positionY) > _totems[i].second.effradius)
                    {
                        ...
                        {
                            _totems[i].first = 0;
                            //TC_LOG_ERROR("player", "%s has lost totem in slot %u! Despawned normally?", me->GetName().c_str(), i);
                            continue;
                        }
        ...
        void UpdateAI(uint32 const diff) override
        {
            ...
            if (GetManaPCT(me) < 99 && !me->isInCombat() && Mana_Timer <= diff) {
        ...
        void OnBotDespawn(Creature* summon)
        {
            ...
            {
                TC_LOG_ERROR("player", "SummonedCreatureDespawn(): Shaman bot %s has despawned summon %s which is not a temp summon or not a totem...", me->GetName().c_str(), summon->GetName().c_str());
                return;
            }
            ...
            switch (totem->m_Properties->Id)
            {
                ...
                case SUMMON_TYPE_TOTEM_AIR:
                    ...
                    break;
                default:
                    TC_LOG_ERROR("player", "SummonedCreatureDespawn(): Shaman bot %s has despawned totem %s with unknown type %u", me->GetName().c_str(), summon->GetName().c_str(), totem->m_Properties->Id);
                    return;
            }
        ...
        void OnBotSummon(Creature* summon)
        {
            ...
            if (!totem || !totem->ToTotem())
            {
                TC_LOG_ERROR("player", "OnBotSummon(): Shaman bot %s has summoned creature %s which is not a temp summon or not a totem...", me->GetName().c_str(), summon->GetName().c_str());
                return;
            }
            ...
            switch (totem->m_Properties->Id)
            {
                ...
                case SUMMON_TYPE_TOTEM_AIR:
                    ...
                    break;
                default:
                    TC_LOG_ERROR("player", "OnBotSummon(): Shaman bot %s has summoned totem %s with unknown type %u", me->GetName().c_str(), summon->GetName().c_str(), totem->m_Properties->Id);
                    return;
            }
            ...
            //TC_LOG_ERROR("player", "shaman bot: summoned %s (type %u) at x='%f', y='%f', z='%f'",
   ...
   /*else */if (spellId == STORMSTRIKE)
        ...
        void UnsummonAll()
        {
            for (uint8 i = 0; i != MAX_TOTEMS; ++i)
            {
                if (_totems[i].first != 0)
                {
                    ...
                    if (!to)
                    {
                        //TC_LOG_ERROR("player", "%s has no totem in slot %u during remove!", me->GetName().c_str(), i);
                        continue;
                    }
                    to->ToTotem()->UnSummon();
                }
        ...
        {
            CommonTimers(diff);
            if (Mana_Timer > diff)              Mana_Timer -= diff;
  ...
  /*Talent*/MANA_TIDE_TOTEM             = lvl >= 40 ? MANA_TIDE_TOTEM_1 : 0;
    ...
    private:
        ...
        uint32 Riptide_Timer, Shock_Timer, Lightning_Bolt_Timer, Searing_Totem_Timer, Totem_of_Wrath_Timer,
            ...
            Hex_Timer, Bloodlust_Timer, Stormstrike_Timer, Mana_Tide_Totem_Timer, Mana_Timer;
