...
std::string BotHelper::GetLocaleStringForTextID(std::string& textValue, uint32 textId, int32 localeIdx)
{
    ...
    {
        TC_LOG_ERROR("player", "botgiver:GetLocaleStringForTextID:: unknown text id: %u!", uint32(textId));
        return textValue;
    }
    ...
    if (localeIdx < 0)
    {
        TC_LOG_ERROR("player", "botgiver:GetLocaleStringForTextID:: unknown locale: %i! Sending default locale text...", localeIdx);
        return textValue;
    }
