...
public:
    ...
    struct bot_druid_ai : public bot_minion_ai
    {
        ...
        bool BuffTarget(Unit* target, uint32 diff)
        {
            ...
            if (!target || !target->IsInWorld() || target->isDead() || me->GetExactDist(target) > 30 || target->isPet()) {
                return false;
            }

            {
                ...
                if (!me->isInCombat() || master->GetMap()->IsRaid())
					if (!HasAuraName(target, MARK_OF_THE_WILD) && !HasAuraName(target, 19740) && !HasAuraName(target, 20217))
...
/*Misc*/uint32 formtimer, ragetimer, Warstomp_Timer, Mana_Timer;
