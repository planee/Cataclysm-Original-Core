...
public:
    ...
    struct paladin_botAI : public bot_minion_ai
    {
        ...
        {
            if (!target || target->isDead() || GC_Timer > diff || Rand() > 20 || target->isPet())
				return false;
			...
			bool mark  = HasAuraName(target, 1126);
            if (kings || might || mark)
                return false;

			if (!kings && !mark && doCast(target, BLESSING_OF_KINGS))
                return true;

			if (!might && !mark && doCast(target, BLESSING_OF_MIGHT))
                return true;
   ...
   /*Hands*/HAND_OF_PROTECTION, HOF, HOS, HANDOFRECKONING,
    /*Misc*/CLEANSE, REDEMPTION, DIVINE_PLEA;
        ...
        uint32 Crusader_cd, Consecration_cd, Exorcism_Timer, Holy_Wrath_Timer, Judge_Timer, HOF_Timer,
            ...
            LOH_Timer, HOJ_Timer, BOP_Timer, AW_Timer, Divine_Plea_Timer, Repentance_Timer, Mana_Timer,
            Rebuke_Timer;
        uint64 HOFGuid;
