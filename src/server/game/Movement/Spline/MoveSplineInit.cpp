...
namespace Movement
{
    ...
    MoveSplineInit::MoveSplineInit(Unit* m, uint32 p_OverrideSplineId) : unit(m)
    {
        args.splineId = p_OverrideSplineId ? p_OverrideSplineId : splineIdGen.NewId();
    ...
    {
        bool petIsInTransport = unit->isPet() && unit->GetTransGUID();
        bool vehicleInWater = unit->IsVehicle() && unit->IsInWater();
        if (generatePath && !petIsInTransport && !vehicleInWater)
        {
