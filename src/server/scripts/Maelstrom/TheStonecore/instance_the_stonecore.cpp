            ...
            void SetData(uint32 type, uint32 data) override
            {
                switch (type)
                {
                    ...
                    case DATA_CORBORUS:
                        ...
                        {
                            PathGenerator l_Path(Corborus);
                            l_Path.CalculatePath(ManaStormPathEvent[3].GetPositionX(), ManaStormPathEvent[3].GetPositionY(), ManaStormPathEvent[3].GetPositionZ());
                            Corborus->GetMotionMaster()->MoveCharge(l_Path, 38.0f, EVENT_CHARGE);
                            ...
                            {
                                Milhouse->AI()->DoAction(ACTION_MANASTORM_PULL);
                                Milhouse->GetMotionMaster()->MoveJump(ManastormJumpEvent, 30.0f, 15.0f, EVENT_JUMP);

                                // Near Cultists too
                                std::list<Creature*> cultistsList;
                                GetCreatureListWithEntryInGrid(cultistsList, Milhouse, NPC_STONECORE_BERSERKER, 50.0f);
                                GetCreatureListWithEntryInGrid(cultistsList, Milhouse, NPC_STONECORE_EARTHSHAPER, 50.0f);
                                GetCreatureListWithEntryInGrid(cultistsList, Milhouse, NPC_STONECORE_WARBRINGER, 50.0f);

                                for (auto&& itr : cultistsList)
                                    itr->GetMotionMaster()->MoveJump(ManastormJumpEvent.GetPositionX() + frand(-5.0f, 5.0f), ManastormJumpEvent.GetPositionY() + frand(-10.0f, 10.0f), ManastormJumpEvent.GetPositionZ(), 30.0f, 15.0f, EVENT_JUMP);
                            }
                        }
                        break;
                }
