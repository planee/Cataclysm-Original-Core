...
enum introEventSpells
{
    ...
    SPELL_BARON_GEDDON_INFERNO_PERIODIC = 74813,
};
...
public:
    ...
    void OnEnterMap(Player* player, uint32 mapId)
    {
        if (mapId == 861)
        {
            // Don't teleport players who are Gamemasters (.gm on)
            if (player->GetTypeId() == TYPEID_PLAYER && (player->isGameMaster())) 
                return;
...
public:
    ...
    class spell_baron_geddon_inferno_SpellScript : public SpellScript
    {
        PrepareSpellScript(spell_baron_geddon_inferno_SpellScript);
        ...
        void HandleDamage(SpellEffIndex effIndex)
        {
            ...
            {
                if (AuraEffect* aurEff = caster->GetAuraEffect(SPELL_BARON_GEDDON_INFERNO_PERIODIC, EFFECT_0, caster->GetGUID()))
                {
                    ...
                    case 3:
                        ...
                        break;
                    case 4:
                        SetHitDamage(damage * 8);
                        break;
                    case 5:
                        SetHitDamage(damage * 16);
                        break;
                    case 6:
                        SetHitDamage(damage * 32);
                        break;
                    case 7:
                        SetHitDamage(damage * 64);
                        break;
                    case 8:
                        SetHitDamage(damage * 128);
                        break;
                    }
                }
            }
        ...
        {
            OnEffectHitTarget += SpellEffectFn(spell_baron_geddon_inferno_SpellScript::HandleDamage, EFFECT_0, SPELL_EFFECT_SCHOOL_DAMAGE);
        }
    };

    SpellScript* GetSpellScript() const override
    {
        return new spell_baron_geddon_inferno_SpellScript();
    }
};
