...
/*######
Live Bait Fishing Quest
######*/

enum eLiveBaitData
{
    QUEST_LIVE_BAIT = 29347,
    SPELL_CRYFISH_CATCH = 99424,
    SPELL_CRYFISH_RIDE_AURA = 99425,
    SPELL_GRIMNURS_BAIT = 99435,
    NPC_COLD_WATER_CRYFISH = 53540,
};

class npc_crayfish : public CreatureScript
{
public:
    npc_crayfish() : CreatureScript("npc_crayfish") { }

    CreatureAI* GetAI(Creature* creature) const
    {
        return new npc_crayfishAI(creature);
    }

    struct npc_crayfishAI : public ScriptedAI
    {
        npc_crayfishAI(Creature* c) : ScriptedAI(c) { }
        ...
        void MoveInLineOfSight(Unit* who)
        {
            ...
            {
                if (player->GetQuestStatus(QUEST_LIVE_BAIT) != QUEST_STATUS_INCOMPLETE)
                    return;
                if (player->HasAura(SPELL_GRIMNURS_BAIT) && !player->HasAura(SPELL_CRYFISH_CATCH) && player->IsInWater())
                {
                    player->CastCustomSpell(SPELL_CRYFISH_CATCH, SPELLVALUE_BASE_POINT0, 1, player, false);
                    for (int i = 0; i < 8; i++)
                    {
                        if (Creature *cryfish = player->SummonCreature(NPC_COLD_WATER_CRYFISH, player->GetPositionX(), player->GetPositionY(), player->GetPositionZ(), 0, TEMPSUMMON_TIMED_DESPAWN, 600000))
                        {
                            cryfish->CastCustomSpell(SPELL_CRYFISH_RIDE_AURA, SPELLVALUE_BASE_POINT0, player->GetTransSeat() + 1, player, TRIGGERED_IGNORE_CASTER_MOUNTED_OR_ON_VEHICLE);
                        }
                    }
                }
                else
...
class spell_78395 : public SpellScriptLoader
{
public:
    spell_78395() : SpellScriptLoader("spell_78395") { }

    class spell_78395_SpellScript : public SpellScript
    {
        PrepareSpellScript(spell_78395_SpellScript);
        ...
        {
            OnEffectHitTarget += SpellEffectFn(spell_78395_SpellScript::HandleActiveObject, EFFECT_1, SPELL_EFFECT_ACTIVATE_OBJECT);
        }
    };

    SpellScript* GetSpellScript() const override
    {
        return new spell_78395_SpellScript();
    }
};
...
## go_loose
######*/
class go_loose : public GameObjectScript
{
public:
    go_loose() : GameObjectScript("go_loose") { }
