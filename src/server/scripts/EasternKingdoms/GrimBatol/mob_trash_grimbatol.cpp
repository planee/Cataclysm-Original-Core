                    ...
                    switch(eventId)
                    {
                        ...
                        case EVENT_PETRIFIED_SKIN:
                            if (Unit* target = me->getVictim())
                                DoCast(target, SPELL_PETRIFIED_SKIN);
                ...
                while(uint32 eventId = events.ExecuteEvent())
                {
                    switch(eventId)
                    {
                        case EVENT_TWILIGHT_BOLT:
                            if (Unit* target = me->getVictim())
                                DoCast(target, SPELL_TWILIGHT_BOLT);
...
// Azureborn Warlord 39909
struct npc_azureborne_warlord : public ScriptedAI
{
    npc_azureborne_warlord(Creature* creature) : ScriptedAI(creature) { }
                ...
                while(uint32 eventId = events.ExecuteEvent())
                {
                    switch(eventId)
                    {
                        case EVENT_DISARMING_BLAZE:
                            if (Unit* target = me->getVictim())
                                DoCast(target, SPELL_DISARMING_BLAZE);
                ...
                while(uint32 eventId = events.ExecuteEvent())
                {
                    switch(eventId)
                    {
                        case EVENT_CRUNCH_ARMOR:
                            if (Unit* target = me->getVictim())
                                DoCast(target, SPELL_CRUNCH_ARMOR);
                ...
                while (uint32 eventId = events.ExecuteEvent())
                {
                    switch (eventId)
                    {
                        case EVENT_MORTAL_STRIKE:
                            if (Unit* target = me->getVictim())
                                DoCast(target, SPELL_MORTAL_STRIKE);
                ...
                while (uint32 eventId = events.ExecuteEvent())
                {
                    switch (eventId)
                    {
                        case EVENT_MOLTEN_BLAST:
                            if (Unit* target = me->getVictim())
                                DoCast(target, SPELL_MOLTEN_BLAST);
                    ...
                    switch (eventId)
                    {
                        case EVENT_CALL_WYRM:
                            ...
                            if (Creature* _wyrm = me->SummonCreature(NPC_TWILIGHT_DRAKE_1, me->GetPositionX(), me->GetPositionY(), me->GetPositionZ() + 20.0f, me->GetOrientation()))
                                _wyrm->GetMotionMaster()->MovePoint(0, me->GetPositionX(), me->GetPositionY(), me->GetPositionZ());
...
struct npc_battered_red_drake : public CreatureAI
{
    npc_battered_red_drake(Creature* creature) : CreatureAI(creature) { }
...
struct npc_battered_red_drake_flyer : public CreatureAI
{
    npc_battered_red_drake_flyer(Creature* creature) : CreatureAI(creature) { }

    TaskScheduler scheduler;
    bool hasTriggered;
    uint64 vehicleOwnerGUID;

    void Reset() override
    {
        me->RemoveFlag(UNIT_NPC_FLAGS, UNIT_NPC_FLAG_SPELLCLICK);
        DoCast(me, SPELL_NET_SUMMON, true);
        hasTriggered = false;
        vehicleOwnerGUID = 0;
    }
    ...
    void OnSpellClick(Unit* clicker, bool& /*result*/) override
    {
        ...
        Talk(urand(0, 6));
        vehicleOwnerGUID = clicker->GetGUID();
        hasTriggered = true;
        me->RemoveFlag(UNIT_NPC_FLAGS, UNIT_NPC_FLAG_SPELLCLICK);

        // Fly path - Fly away after remove passenger
        ...
        for (uint8 i = 0; i < 17; i++)
            init.Path().push_back(G3D::Vector3(batteryDragonFlyPath[i].GetPositionX() + frand(-8.0f, 8.0f), batteryDragonFlyPath[i].GetPositionY() + frand(-7.5f, 7.5f), batteryDragonFlyPath[i].GetPositionZ()));

        init.Path().push_back(G3D::Vector3(me->GetHomePosition().GetPositionX(), me->GetHomePosition().GetPositionY(), me->GetHomePosition().GetPositionZ() + 7.5f));
        init.SetSmooth();
        init.SetFly();
        init.Launch();
        ...
        {
            if (Unit* passenger = ObjectAccessor::GetUnit(*me, vehicleOwnerGUID))
...
// Net 42570
struct npc_battered_red_drake_net : public CreatureAI
{
    npc_battered_red_drake_net(Creature* creature) : CreatureAI(creature) { }
    ...
    void JustDied(Unit* /*killer*/) override
    {
        if (Unit* owner = ObjectAccessor::GetUnit(*me, ownerGUID))
            owner->RemoveAurasDueToSpell(SPELL_NET_EFF);
...
// Meat Grinder 76414
class spell_twilight_enforcer_meat_grinder : public AuraScript
{
    PrepareAuraScript(spell_twilight_enforcer_meat_grinder);

    void OnRemove(AuraEffect const* /*aurEff*/, AuraEffectHandleModes /*mode*/)
    {
        if (Unit* owner = GetOwner()->ToUnit())
            owner->CastSpell(owner, SPELL_DIZZY, true);
    }

    void Register() override
    {
        OnEffectRemove += AuraEffectRemoveFn(spell_twilight_enforcer_meat_grinder::OnRemove, EFFECT_0, SPELL_AURA_DUMMY, AURA_EFFECT_HANDLE_REAL);
    }
};

// Shadow Weave 90673
class spell_twilight_shadow_weaver_shadow_weave : public SpellScript
{
    PrepareSpellScript(spell_twilight_shadow_weaver_shadow_weave);

    void HandleScript(SpellEffIndex /*effIndex*/)
    {
        if (Unit* caster = GetCaster())
        {
            caster->CastSpell(caster, SPELL_SHADOW_WEAVE_DUMMY, true);
            caster->CastSpell(caster, SPELL_SHADOW_WEAVE_DMG, false);
        }
    }

    void Register() override
    {
        OnEffectHitTarget += SpellEffectFn(spell_twilight_shadow_weaver_shadow_weave::HandleScript, EFFECT_0, SPELL_EFFECT_SCRIPT_EFFECT);
    }
};

// Electric Blast 76579
class spell_twilight_thundercaller_electric_blast : public SpellScript
{
    PrepareSpellScript(spell_twilight_thundercaller_electric_blast);

    void HandleScript()
    {
        if (Unit* caster = GetCaster())
        {
            uint8 stackAmount = caster->GetAura(SPELL_OVERCHARGE) ? caster->GetAura(SPELL_OVERCHARGE)->GetStackAmount() : 0;
    ...
    void Register() override
    {
        BeforeHit += SpellHitFn(spell_twilight_thundercaller_electric_blast::HandleScript);
    }
};

// Net 79377
class spell_grim_batol_net : public AuraScript
{
    PrepareAuraScript(spell_grim_batol_net);
    ...
    void OnRemove(AuraEffect const* /*aurEff*/, AuraEffectHandleModes /*mode*/)
    {
        if (Creature* owner = GetOwner()->ToCreature())
        {
            ...
            owner->RemoveFlag(UNIT_FIELD_FLAGS, UNIT_FLAG_NOT_SELECTABLE);
        }
    }

    void Register() override
    {
        OnEffectRemove += AuraEffectRemoveFn(spell_grim_batol_net::OnRemove, EFFECT_0, SPELL_AURA_MOD_ROOT, AURA_EFFECT_HANDLE_REAL);
    }
};
...
void AddSC_mob_trash_grim_batol()
{
    ...
    new creature_script<npc_azureborne_warlord>("npc_azureborne_warlord");
    ...
    new creature_script<npc_battered_red_drake>("npc_battered_red_drake");
    new creature_script<npc_battered_red_drake_flyer>("npc_battered_red_drake_flyer");
    new creature_script<npc_battered_red_drake_net>("npc_battered_red_drake_net");

    new aura_script<spell_twilight_enforcer_meat_grinder>("spell_twilight_enforcer_meat_grinder");
    new spell_script<spell_twilight_shadow_weaver_shadow_weave>("spell_twilight_shadow_weaver_shadow_weave");
    new spell_script<spell_twilight_thundercaller_electric_blast>("spell_twilight_thundercaller_electric_blast");
    new aura_script<spell_grim_batol_net>("spell_grim_batol_net");
}
