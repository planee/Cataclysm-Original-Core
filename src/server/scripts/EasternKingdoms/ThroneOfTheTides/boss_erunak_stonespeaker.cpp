...
public:
    ...
    struct boss_erunak_stonespeakerAI : public ScriptedAI
    {
        ...
        void Reset()
        {
            ...
            CastTimer = 3000;
            start = false;
            noValidTargetWhileControlled = false;
            _phase = PHASE_ERUNAK;
        ...
        void EnslaveCheckLife()
        {
            ...
            {
                if (enslaveTarget->HealthBelowPct(50) || (enslaveTarget->GetTypeId() == TYPEID_PLAYER && !enslaveTarget->HasAura(SPELL_ENSLAVE) || noValidTargetWhileControlled))
                {
                    ...
                    if (Creature *mindbender = Unit::GetCreature((*me), instance->GetData64(DATA_MINDBENDER_GHURSHA)))
                    {
                        ...
                        enslaveTargetGUID = 0;
                        if (noValidTargetWhileControlled)
                        {
                            mindbender->Kill(enslaveTarget);
                            noValidTargetWhileControlled = false;
                        }
                    }
                }
                        ...
                        case PHASE_PLAYER_CONTROLLED:
                        {
                            if (CastTimer <= diff)
                            {
                                ...
                                if (Unit *target = SelectTarget(SELECT_TARGET_RANDOM, 0, NotCharmedTargetSelector()))
                                {
                                    ...
                                    noValidTargetWhileControlled = true;
                            }
                            break;
                        }
                        case PHASE_ERUNAK:
    ...
    private :
        ...
        uint8 _phase;
        bool noValidTargetWhileControlled;
    };
        ...
        void InitializeAI() override
        {
            scheduler.Schedule(Milliseconds(1000), [this](TaskContext context)
            {
                if (Creature *erunak = Unit::GetCreature((*me), instance->GetData64(DATA_ERUNAK_STONESPEAKER)))
                {
                    ...
                    // After exiting vehicle he consistently tries to evade to this unloaded grid and i have no idea why...
                    me->GetMap()->LoadGrid(-614.138f, 808.34f);
                }
            });
        }
