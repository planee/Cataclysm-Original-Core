...
class instance_deadmines : public InstanceMapScript
{
    public:
        ...
        struct instance_deadmines_InstanceMapScript : public InstanceScript
        {
            ...
            void OnCreatureCreate(Creature* creature)
            {
                ...
                switch (creature->GetEntry())
                {
                    ...
                    case 46906: // Slinky Sharpshiv
                        ...
                        break;
                    case 46612: // Lieutenant Horatio Laine, for some reason above check still shows to horde ^^, so let's make a second check against hordes.
                        if (TeamInInstance != ALLIANCE)
                            creature->SetVisible(false);
                        break;
                    case 46614:
                   ...
                   case NPC_VAPOR_VALVE:
                       steamValveList.push_back(creature->GetGUID());
                       creature->SetFlag(UNIT_FIELD_FLAGS, UNIT_FLAG_NOT_SELECTABLE);
                       break;
                   case NPC_FOE_REAPER_5000:
                       FoeReaperGUID = creature->GetGUID();
                       break;
                }
                CheckRecquirements();
            }
            ...
            {
                if (creature->GetEntry() == NPC_VAPOR_VALVE)
                    steamValveList.remove(creature->GetGUID());
            }
            ...
            void OnGameObjectCreate(GameObject* go)
            {
                switch (go->GetEntry())
                {
                    ...
                    // case GO_FOE_ENTRANCE: // Prevent being open by default
                        AddDoor(go, true);
                        break;
                    case GO_FOE_EXIT:
                    ...
                    case GO_IRON_CLAD_DOOR:
                        ...
                        AddDoor(go, true);
                        break;
                    case GO_EVENT_CANNON:
                        EventCannonGUID = go->GetGUID();
                        break;
            ...
            uint64 GetData64(uint32 data) const
            {
                switch (data)
                {
                    ...
                    case BOSS_FOE_REAPER_5000_DATA:
                        return FoeReaperGUID;
                    default:
                        return 0;
                }
            }
                ...
                switch (type)
                {
                    ...
                    case DATA_START_VANCLEEF_EVENT:
                    {
                        ...
                        if (GameObject *go = instance->GetGameObject(doorDefiasCanonGUID))
                            go->SetGoState(GO_STATE_READY);
                        if (GameObject *go = instance->GetGameObject(EventCannonGUID))
                            go->SetFlag(GAMEOBJECT_FLAGS, GO_FLAG_IN_USE); // make sure cannon is still not usable maybe flag has been reset after crash/restart
                        ...
                        case EVENT_VANCLEEF_EVENT:
                        {
                            switch (eventStep)
                            {
                                ...
                                case 2:
                                {
                                    ...
                                    for (uint64 guid : steamValveList)
                                    {
                                        if (Creature* valve = instance->GetCreature(guid))
                                        {
                                            valve->RemoveFlag(UNIT_FIELD_FLAGS, UNIT_FLAG_NOT_SELECTABLE);
                                            valve->AddAura(94557, valve);
                                        }
                                    }
        ...
        private:
            ...
            uint64 FoeReaperGUID;
            uint64 EventCannonGUID;
            EventMap _events;
            ...
            bool achFailed;
            std::list<uint64> steamValveList;
        };
