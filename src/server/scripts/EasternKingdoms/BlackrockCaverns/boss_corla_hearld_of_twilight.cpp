...
class boss_corla_herald_of_twilight : public CreatureScript
{
    public:
        ...
        struct boss_corla_herald_of_twilightAI : public BossAI
        {
            ...
            {
                uint8 mode = IsHeroic() ? 2 : 1;

                for (uint8 i = 0; i <= mode; i++)
            ...
            void Reset() override
            {
                ...
                for (uint8 i = 0; i <= mode; i++)
                    NetherEssenceTrigger[i]->AI()->DoAction(ACTION_TRIGGER_STOP_CHANNELING);
...
// Twilight Zealot 50284
struct npc_twilight_zealot : public ScriptedAI
{
    npc_twilight_zealot(Creature* creature) : ScriptedAI(creature), intialized(false) {}
    ...
    {
        if (actionId == ACTION_EVOLVED && !intialized)
        {
            intialized = true;
...
// Corla Quest Bang 49526
struct npc_corla_netheressence_trigger : public ScriptedAI
{
    npc_corla_netheressence_trigger(Creature* creature) : ScriptedAI(creature) { }
    ...
    void UpdateAI(uint32 diff) override
    {
        ...
        {
            if (me->GetInstanceScript() && me->GetInstanceScript()->GetBossState(DATA_CORLA) == IN_PROGRESS)
            {
                    ...
                    if (evolve->GetStackAmount() > 99)
                    {
                        ...
                        if (InstanceScript* instance = me->GetInstanceScript())
                        {
                            if (Creature* Corla = ObjectAccessor::GetCreature(*me, instance->GetData64(DATA_CORLA)))
                            {
                                ...
                                else if (channelTarget->ToCreature())
                                {
                                    channelTarget->ToCreature()->AI()->DoAction(ACTION_EVOLVED);
...
// Evolution 75697, 87378
class spell_korla_evolution : public SpellScript
{
    PrepareSpellScript(spell_korla_evolution);

    void HandleEffectHitTarget(SpellEffIndex eff_idx)
    {
        if (GetCaster() && GetCaster()->GetInstanceScript() && GetCaster()->GetInstanceScript()->GetBossState(DATA_CORLA) != IN_PROGRESS)
            PreventHitAura();
    }

    void Register() override
    {
        OnEffectHitTarget += SpellEffectFn(spell_korla_evolution::HandleEffectHitTarget, EFFECT_0, SPELL_EFFECT_APPLY_AURA);
    }
};
...
void AddSC_boss_corla_herald_of_twilight()
{
    ...
    new creature_script<npc_twilight_zealot>("npc_twilight_zealot");
    new creature_script<npc_corla_netheressence_trigger>("npc_corla_netheressence_trigger");
    new aura_script<spell_twilight_evolution>("spell_twilight_evolution");
    new spell_script<spell_korla_evolution>("spell_korla_evolution");
}