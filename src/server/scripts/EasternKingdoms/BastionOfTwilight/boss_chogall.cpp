...
enum Achievements
{
    DATA_IS_ABYSS_GAZE_BACK_TO_YOU_ELIGIBLE = 1,
};
    ...
    else if (oldvalue < 25 && newvalue >=25)
    {
        ...
        if (newvalue >= 30)
        {
            if (InstanceScript* instance = player->GetInstanceScript())
                if (Creature* chogall = ObjectAccessor::GetCreature(*player, instance->GetData64(DATA_CHOGALL)))
                    chogall->AI()->SetData(DATA_IS_ABYSS_GAZE_BACK_TO_YOU_ELIGIBLE, 0);
        }
    }
}
...
class boss_chogall : public CreatureScript
{
    public:
        ...
        struct boss_chogallAI : public BossAI
        {
            ...
            bool bPhase2;
            uint8 uiOrder; // 0 | 1
            uint32 isAbyssGazeBackToYouEligible;
            ...
            void Reset() override 
            {
                ...
                events.Reset();
                summons.DespawnAll();
                isAbyssGazeBackToYouEligible = 1;
            ...
            void SetData(uint32 type, uint32 data) override
            {
                switch (type)
                {
                        case DATA_IS_ABYSS_GAZE_BACK_TO_YOU_ELIGIBLE:
                            isAbyssGazeBackToYouEligible = data;
                            break;
                    default:
                        break;
                }
            ...
            {
                if (type == DATA_IS_ABYSS_GAZE_BACK_TO_YOU_ELIGIBLE)
                    return isAbyssGazeBackToYouEligible;
...
// Consume blood of the old god 82630
class spell_chogall_consume_blood_of_old_god : public SpellScript
{
    PrepareSpellScript(spell_chogall_consume_blood_of_old_god);

    void SelectTargets(std::list<WorldObject*>&targets)
    {
        targets.remove_if([=](WorldObject* target) { return target->GetEntry() != NPC_CORRUPTING_ADHERENT; });
    }

    void SelectTargetsStuned(std::list<WorldObject*>&targets)
    {
        targets.remove_if([=](WorldObject* target) { return target && target->ToUnit() && target->ToUnit()->GetTypeId() != TYPEID_PLAYER; });
    }

    void Register() override
    {
        OnObjectAreaTargetSelect += SpellObjectAreaTargetSelectFn(spell_chogall_consume_blood_of_old_god::SelectTargets, EFFECT_0, TARGET_UNIT_SRC_AREA_ENTRY);
        OnObjectAreaTargetSelect += SpellObjectAreaTargetSelectFn(spell_chogall_consume_blood_of_old_god::SelectTargets, EFFECT_1, TARGET_UNIT_SRC_AREA_ENTRY);
        OnObjectAreaTargetSelect += SpellObjectAreaTargetSelectFn(spell_chogall_consume_blood_of_old_god::SelectTargetsStuned, EFFECT_2, TARGET_UNIT_SRC_AREA_ENTRY);
    }
};

class achievement_abyss_will_gaze_back_into_you : public AchievementCriteriaScript
{
public:
    achievement_abyss_will_gaze_back_into_you() : AchievementCriteriaScript("achievement_abyss_will_gaze_back_into_you") { }
    ...
    bool OnCheck(Player* /*source*/, Unit* target)
    {
        ...
        return target->GetAI()->GetData(DATA_IS_ABYSS_GAZE_BACK_TO_YOU_ELIGIBLE) == 1;
    }
};
...
void AddSC_boss_chogall()
{
    ...
    new spell_script<spell_chogall_consume_blood_of_old_god>("spell_chogall_consume_blood_of_old_god");
    new achievement_abyss_will_gaze_back_into_you();
}