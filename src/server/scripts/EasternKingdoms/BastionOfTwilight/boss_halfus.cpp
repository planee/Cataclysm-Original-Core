...
class boss_halfus_wyrmbreaker : public CreatureScript
{
    public:
        ...
        struct boss_halfus_wyrmbreakerAI : public BossAI
        {
            ...
            bool bWhelps;
            uint8 whelpcount;
            uint8 chosenCombinationForWeek;
            uint32 theOnlyEscapeCountdown = 0;
            bool theOnlyEscape = true;
            bool lockTimerTheOnlyEscape = false;

            void InitializeAI() override
            {
                chosenCombinationForWeek = urand(0,9);
                BossAI::InitializeAI();
            }

            uint32 GetData(uint32 id) const override
            {
                if (id == DATA_HALFUS_ONLY_ESCAPE)
                    return theOnlyEscape;
            ...
            void Reset() override
            {
                _Reset();
                theOnlyEscape = true;
                lockTimerTheOnlyEscape = false;
                theOnlyEscapeCountdown = 0;
                ...
                {
                    switch (chosenCombinationForWeek)
                    {
                ...
                switch (action)
                {
                    ...
                    case ACTION_WHELP_DIED:
                        ...
                        if (whelpcount == 8)
                        {
                            ...
                            if (theOnlyEscapeCountdown == 0)
                                theOnlyEscapeCountdown = 10 * IN_MILLISECONDS;
                            else // timer was already initiated, achievement is good to go
                                lockTimerTheOnlyEscape = true;

                        }
                        break;
                    case ACTION_DRAKE_DIED:
                        if (theOnlyEscapeCountdown == 0)
                            theOnlyEscapeCountdown = 10 * IN_MILLISECONDS;
                        else // timer was already initiated, achievement is good to go
                            lockTimerTheOnlyEscape = true;
            ...
            void UpdateAI(uint32 diff) override
            {
                ...
                if (theOnlyEscapeCountdown > 0 && !lockTimerTheOnlyEscape)
                {
                    if (theOnlyEscapeCountdown <= diff)
                        theOnlyEscape = false; // dont set countdown to 0.
                    else
                        theOnlyEscapeCountdown -= diff;
                }
            ...
            void DoAction(int32 action) override
            {
                if (action == ACTION_WHELPS_RELEASE)
                {
                    ...
                    if (Creature* proto = me->FindNearestCreature(NPC_PROTO_BEHEMOTH, 200.0f))
                    {
                        ...
                        // Doing this for the sake of speeding things up, let spellcast be done for visual shenanigans and just aura the stacks on the proto behemoth
                        // TODO: Debug this spell and see why stacks are not applied?
                        me->AddAura(SPELL_ATROPHIC_POISON, proto);
                    }
    ...
    public:
        ...
        struct npc_halfus_dragonAI : public ScriptedAI 
        {
            ...
            void JustDied(Unit* /*killer*/) override
            {
                ...
                if (Creature* halfus = ObjectAccessor::GetCreature(*me, me->GetInstanceScript()->GetData64(DATA_HALFUS)))
                    halfus->GetAI()->DoAction(ACTION_DRAKE_DIED);
            }
        };
...
public:
    ...
    bool OnCheck(Player* /*source*/, Unit* target)
    {
        ...
        return target->GetAI()->GetData(DATA_HALFUS_ONLY_ESCAPE) == 1;
    }
};
