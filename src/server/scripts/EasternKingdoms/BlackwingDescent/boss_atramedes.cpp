...
enum Points
{
    POINT_INTRO_LAND            = 1000,
    POINT_CENTER_TAKEOFF        = 1001,
    POINT_CENTER_AIR            = 1002,
    POINT_CENTER_LAND           = 1003,
};
...
uint32 const AtramedesIntroFlyPathSize = 3;
const G3D::Vector3 AtramedesIntroFlyPath[AtramedesIntroFlyPathSize] =
{
    {387.687f, -251.759f, 129.786f},
    {337.967f, -225.78f, 122.211f},
    {261.698f, -224.221f, 106.788f},
};
    ...
    struct boss_atramedesAI : public BossAI
    {
        ...
        void Reset()
        {
            ...
            else
            {
                ...
                me->GetMotionMaster()->MoveSmoothPath(AtramedesIntroFlyPath, AtramedesIntroFlyPathSize);
                introTimer = 0;
            }
        ...
        {
            if (spell->Id == SPELL_RESONATING_CLASH_PLR) // Condition based
            {
                if (!airPhase)
                {
                    me->RemoveAurasDueToSpell(SPELL_SEARING_FLAME);
                    me->RemoveAurasDueToSpell(SPELL_SEARING_FLAME_DMG);
                    //DoCast(me, SPELL_VERTIGO, true); // CASTED BY SPELLSCRIPT
                }
                else
                {
        ...
        void EnterCombat(Unit * /*who*/)
        {
            ...
            if (Creature* maloriak = me->FindNearestCreature(NPC_MALORIAK_EVENT, 200.0f))
                maloriak->AI()->DoAction(ACTION_DESPAWN_EVENT);
        ...
        void MovementInform(uint32 type, uint32 id) override
        {
            ...
            {
                if (id == POINT_CENTER_TAKEOFF)
                {
                    me->SetHover(true);
                    me->SetDisableGravity(true);
                    me->SetCanFly(true);
                    me->SetByteFlag(UNIT_FIELD_ANIM_TIER, 3, UNIT_BYTE1_FLAG_ALWAYS_STAND | UNIT_BYTE1_FLAG_HOVER);
                    Position fightPos = { me->GetPositionX(), me->GetPositionY(), me->GetPositionZ() + 20.0f };
                    me->GetMotionMaster()->MoveTakeoff(POINT_CENTER_AIR, fightPos); // nothing to be done on this point.
            ...
            else if (type == SPLINE_MOTION_TYPE && id == AtramedesIntroFlyPathSize - 1 && !introDone)
            {
        ...
        void UpdateAI(const uint32 diff)
        {
            if (!introDone && introTimer)
            {
                if (introTimer <= diff)
                {
                    ...
                    me->GetMotionMaster()->MoveLand(POINT_INTRO_LAND, homePos);
                }
            ...
            if (uint32 eventId = events.ExecuteEvent())
            {
                switch(eventId)
                {
                    ...
                    case EVENT_AIR_PHASE:
                        ...
                        me->GetMotionMaster()->MovePoint(POINT_CENTER_TAKEOFF, centerPos);
                        break;
                ...
                case EVENT_GROUND_PHASE:
                    ...
                    me->SetReactState(REACT_AGGRESSIVE);
                    me->GetMotionMaster()->Clear(true);
                    me->GetMotionMaster()->MoveLand(POINT_CENTER_LAND, centerPos);
                    events.Reset();
                    events.ScheduleEvent(EVENT_MODULATION, urand(10000, 12000));
...
class spell_resonating_clash_aur : public SpellScriptLoader
{
public:
    spell_resonating_clash_aur() : SpellScriptLoader("spell_resonating_clash_aur") { }

    class spell_resonating_clash_aur_SpellScript : public SpellScript
    {
        PrepareSpellScript(spell_resonating_clash_aur_SpellScript);

        void HandleEffect(SpellEffIndex /*effIndex*/)
        {
            if (Unit* target = GetHitUnit())
                target->RemoveAura(SPELL_NOISY);
        }

        void Register()
        {
            OnEffectHitTarget += SpellEffectFn(spell_resonating_clash_aur_SpellScript::HandleEffect, EFFECT_2, SPELL_EFFECT_SCRIPT_EFFECT);
        }
    };

    SpellScript* GetSpellScript() const
    {
        return new spell_resonating_clash_aur_SpellScript();
    }
};


class spell_resonating_clash : public SpellScriptLoader
{
public:
    spell_resonating_clash() : SpellScriptLoader("spell_resonating_clash") { }

    class spell_resonating_clash_SpellScript : public SpellScript
    {
        PrepareSpellScript(spell_resonating_clash_SpellScript);
        ...
        void HandleDummy(SpellEffIndex effIndex)
        {
            if (Unit* caster = GetCaster())
                if (Unit* target = GetHitUnit())
                    if (target->GetAI()->GetData(DATA_IS_AIR_PHASE) == 0)
                     target->CastSpell(target, SPELL_VERTIGO, true);
        }

        void Register() override
        {
            OnEffectHitTarget += SpellEffectFn(spell_resonating_clash_SpellScript::HandleDummy, EFFECT_1, SPELL_EFFECT_SCRIPT_EFFECT);
        }
    };

    SpellScript* GetSpellScript() const override
    {
        return new spell_resonating_clash_SpellScript();
    }
};
...
public:
    ...
    struct npc_ancient_dwarven_shieldAI : public PassiveAI
    {
        ...
        void OnSpellClick(Unit* player, bool& result)
        {
            ...
            DoCast(SPELL_RESONATING_CLASH); // Interrupts cast + triggers vertigo
            DoCast(player, SPELL_RESONATING_CLASH_AUR, true); // Clears sound + noisy
            player->CastSpell(player, SPELL_RESONATING_CLASH_PLR, true); // adds the "you are resonating" aura on player
            DoCast(SPELL_SONIC_SHIELD); // shield destruction animation
...
class go_ancient_bell : public GameObjectScript
{
    bool OnGossipHello(Player* player, GameObject* pGO)
    {
        ...
        {
            // Load atramedes spawn pos
            player->GetMap()->LoadGrid(385.094f, -315.484f);
...
class npc_maloriak_atramedes_event : public CreatureScript
{
    struct npc_maloriak_atramedes_eventAI : public ScriptedAI
    {
        ...
        {
            if (id == ACTION_DESPAWN_EVENT)
            {
                if (Creature *lunarCol = me->FindNearestCreature(NPC_LUNAR_COLUMN, 200.0f))
                    lunarCol->CastSpell(lunarCol, SPELL_LUNAR_COLUMN_VISUAL, true);
                if (GameObject *gob = me->FindNearestGameObject(GOB_THRONE, 200.0f))
                    gob->Delete();
                if (GameObject *gob = me->FindNearestGameObject(GO_ANCIENT_BELL, 200.0f))
        ...
        void UpdateAI(const uint32 diff)
        {
            if (!start)
            {
                if (mui_start <= diff)
                {
                    if (!me->FindNearestCreature(43125, 200.0f, true) && !me->FindNearestCreature(43128, 200.0f, true) && !me->FindNearestCreature(43129, 200.0f, true)
                        && !me->FindNearestCreature(43119, 200.0f, true) && !me->FindNearestCreature(43127, 200.0f, true) && !me->FindNearestCreature(43122, 200.0f, true)
                        && !me->FindNearestCreature(43130, 200.0f, true) && !me->FindNearestCreature(43126, 200.0f, true))
                    {
                        InstanceScript* instance = me->GetInstanceScript();
                        if (instance)
                        {
                            // Skip intro if already done or server crashed
                            if (instance->GetData(DATA_NEFARIOUS_ATRAMEDES_INTRO_STATE) == DONE)
                            {
                                if (Creature *lunarCol = me->FindNearestCreature(NPC_LUNAR_COLUMN, 200.0f))
                                    lunarCol->CastSpell(lunarCol, SPELL_LUNAR_COLUMN_VISUAL, true);
                                if (GameObject *gob = me->FindNearestGameObject(GOB_THRONE, 200.0f))
                            ...
                            instance->SetData(DATA_NEFARIOUS_ATRAMEDES_INTRO_STATE, DONE);
                        }
                        // Enable the bell asap, no one needs to listen to this stuff all the time.
                        if (GameObject *gob = me->FindNearestGameObject(GO_ANCIENT_BELL, 200.0f))
                            gob->RemoveFlag(GAMEOBJECT_FLAGS, GO_FLAG_IN_USE);
                        start = true;
                        canTalk = true;
                        ...
                        if (Creature *nefarius = me->SummonCreature(NPC_INTRO_NEFARIAN_ATRAMEDES, 150.7813f, -231.1962f, 75.53674f, 0.0f))
                        {
            ...
            if (canTalk)
            {
                if (textCooldown <= diff)
                {
                    ...
                    {
                    case 0: // 43396
                        if (Creature *nefarius = me->FindNearestCreature(NPC_INTRO_NEFARIAN_ATRAMEDES, 200.0f))
                            nefarius->AI()->Talk(0);
                        textCooldown = 9000;
                        break; // 22:25:24.197
                    case 1:
                    ...
                    case 6:
                        ...
                        break;
                    case 7:
                        if (Creature *atramedes = me->FindNearestCreature(NPC_LITTLE_ATRAMEDES, 200.0f))
...
void AddSC_boss_atramedes()
{
    ...
    new spell_resonating_clash_aur();
    new spell_resonating_clash();
