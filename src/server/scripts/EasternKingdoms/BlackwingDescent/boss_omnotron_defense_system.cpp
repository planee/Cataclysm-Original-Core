...
{
    EMOTE_SHIELD                = 0,
    SAY_ARCANOTRON_ACTIVATE     = 5,
    SAY_ELECTRON_ACTIVATE       = 2,
    SAY_MAGMATRON_ACTIVATE      = 3,
    SAY_TOXITRON_ACTIVATE       = 4,
    ...
    SAY_DEATH                   = 1,
    SAY_NEF_AGGRO               = 0,
    SAY_NEF_ARCANOTRON          = 1,
    SAY_NEF_DEATH_HEROIC        = 2,
    SAY_NEF_MAGMATRON           = 3,
    SAY_NEF_ELECTRON            = 4,
    SAY_NEF_TOXITRON            = 5,
    SAY_NEF_DEATH               = 6
    ...
    public:
        ...
        struct boss_omnotronAI : public BossAI
        {
            ...
            void Reset() override
            {
                ...
                started = false;
                if (Creature* tron = Creature::GetCreature(*me, instance->GetData64(bots[1].data)))
                {
                    me->SetMaxHealth(tron->GetMaxHealth());
                    me->SetFullHealth();
                }
            }
            ...
            void InitBots()
            {
                ...
                if (Creature* tron = Creature::GetCreature(*me, instance->GetData64(bots[currentBot].data)))
                {
                    ...
                    tron->SetMaxPower(POWER_ENERGY, 100);
...
class npc_victor_nefarius_omnotron : public CreatureScript
{
    public:
        ...
        struct npc_victor_nefarius_omnotronAI : public ScriptedAI
        {
            ...
            {
                events.Reset();
                if (instance && instance->GetData(DATA_NEFARIOUS_OMNONTRON_INTRO_STATE) == NOT_STARTED)
                    events.ScheduleEvent(EVENT_INTRO, 5000);
                _canCast = IsHeroic() ? true : false;
            }
                ...
                while (uint32 eventId = events.ExecuteEvent())
                {
                    switch (eventId)
                    {
                        case EVENT_INTRO:
                            Talk(SAY_NEF_AGGRO);
                            if (instance)
                                instance->SetData(DATA_NEFARIOUS_OMNONTRON_INTRO_STATE, DONE);
                            break;
                        case EVENT_DEATH:                      
                            Talk(IsHeroic() ? SAY_NEF_DEATH_HEROIC : SAY_NEF_DEATH);
                            me->DespawnOrUnsummon(5000);
                            break;
